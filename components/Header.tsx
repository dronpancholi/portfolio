...
export default function Header(){
  ...
  const [animPhase, setAnimPhase] = useState<"idle"|"collapsing"|"expanding">("idle");
  ...
  const onPillClick = useCallback(() => {
    if (!isAtTop){
      if (expanded){
        ...
        setTimeout(() => setExpanded(false), 140); // wait for fade-out
      } else {
        setExpanded(true);
    ...
    <div className="fixed top-3 sm:top-4 left-1/2 -translate-x-1/2 z-50 px-3 sm:px-0 w-full flex justify-center pointer-events-none">
      <motion.header
        ...
        variants={{
          ...
          expanded:  { padding:"10px 18px", scale:0.97 },
        ...
        <AnimatePresence initial={false}>
          {(state==="top" || state==="expanded") && (
            <motion.nav
              ...
              className="flex items-center font-medium text-neutral-800 select-none"
